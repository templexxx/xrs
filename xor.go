...
func xorBase(dst []byte, src [][]byte) {
	...
	for start < size {
		...
		if end <= size {
			xorPartBase(start, end, dst, src)
			start = start + do
		} else {
			xorPartBase(start, size, dst, src)
			start = size
		}
	}
}
...
func xorBytesBase(dst, a, b []byte, size int) {
...
func xorPartBase(start, end int, dst []byte, src [][]byte) {
	...
	for i := 2; i < len(src); i++ {
		xorBytesBase(dst[start:end], dst[start:end], src[i][start:end], end-start)
	}
}
